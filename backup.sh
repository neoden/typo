...
check_param "APP_NAME"

if [ -z "${DB_NAME}" ]; then DB_NAME=${APP_NAME}; fi
if [ -z "${DB_USER}" ]; then DB_USER=${DB_NAME}; fi
if [ -z "${DB_PASSWORD}" ]; then DB_PASSWORD=; fi
if [ -z "${DAYS_TO_KEEP_LOCAL_BACKUPS}" ]; then DAYS_TO_KEEP_LOCAL_BACKUPS=7; fi
